...
--- @release   2026-08-23
---
--- @note      LaTeX output requires \usepackage[most]{tcolorbox}, \usepackage{environ},
---            \usepackage{keyval}, and the custom \begin{pullquote} environment defined in the preamble.
---            Typst output requires the custom #let pullquote(...) function defined in your Typst template.
...
if PANDOC_READER_OPTIONS and PANDOC_READER_OPTIONS.extensions then
  ...
  if not (ext:includes('fenced_divs') and ext:includes('bracketed_spans')) then
    io.stderr:write('[pullquote] Warning: Required extensions "fenced_divs" or "bracketed_spans" are disabled.\n')
  end
end

-- ==============================================================================
-- SECTION 1: DATA DICTIONARIES & STATE CONFIGURATION
-- ==============================================================================

-- Configuration State (Clean String Fallbacks for Typst)
local typst_fonts = {
...
local pq_sizes = {
  ...
  ['pq-size-4xl']    = { tex = '\\Huge',                             css = '2.4883em' },
  ['pq-size-5xl']    = { tex = '\\fontsize{30pt}{36pt}\\selectfont', css = '2.986em' }
}
...
-- ==============================================================================
-- SECTION 2: COLOR PARSING & HELPER FUNCTIONS
-- ==============================================================================
...
local function resolve_single_color(input)
  ...
  if clean_input:match('^[a-zA-Z%-]+$') then
    error(string.format('\n\n[pullquote] CRITICAL ERROR: Undefined color keyword "%s".\nColor must be a valid standard CSS keyword, a Hex code (e.g. #FF0000), or valid cross-platform mixing syntax.\nHalting compilation to prevent LaTeX engine crash.\n', clean_input))
  end

  return nil, nil, false
end

-- Format for LaTeX xcolor (Safe prefixed string) or pass-through for natively mixed colors (e.g. Maroon!30)
local function format_tex_color(c)
  if not c then return nil end
  c = c:match("^%s*(.-)%s*$")
  if c:find('!') then return c end -- Let xcolor handle it natively
  local css_val, tex_val, is_hex = resolve_single_color(c)
  if is_hex then return "HEX" .. tex_val end -- Use safe HEX prefix (no braces)
  return c
end

-- Format for HTML CSS (e.g., #FF0000) or CSS color-mix
...
-- Format for Typst (e.g., rgb("#FF0000")) or Typst color.mix
...
local function handle_pullquote(el)
  ...
  -- 2. Extract active font styles from classes
  ...
  if FORMAT:match 'latex' then
    ...
    -- Inline Color Definition Helper (from fonts-and-alignment logic)
    local function process_tex_color(input_color, option_key, temp_color_name)
      ...
      if c:find('!') then
        ...
        if is_hex then
          -- Define the color inside the local group
    ...
    if skip     then table.insert(options, "skip=" .. skip) end
    if barwidth then table.insert(options, "barwidth=" .. barwidth:gsub("px", "pt")) end

    local tex_size_str = size_key and pq_sizes[size_key] and pq_sizes[size_key].tex or "\\large"
  ...
  elseif FORMAT:match 'html' then
    ...
    local final_color = format_html_color(color) or "#555555"
    ...
    if boxalign_key and pq_box_aligns[boxalign_key] then margin = pq_box_aligns[boxalign_key].html_margin end
    table.insert(styles, "margin: " .. margin .. " !important;")

    local final_size = "1.2em"
    if size_key and pq_sizes[size_key] then final_size = pq_sizes[size_key].css end
  ...
  elseif FORMAT:match 'typst' then
    ...
    if width    then table.insert(args, "width: " .. width) end
    if color    then table.insert(args, "color: " .. format_typst_color(color)) end
    if barwidth then table.insert(args, "barwidth: " .. barwidth:gsub("px", "pt")) end
    if barcolor then table.insert(args, "barcolor: " .. format_typst_color(barcolor)) end

    if size_key and pq_sizes[size_key] then
      table.insert(args, "size: " .. pq_sizes[size_key].css)
    end
    if align_key and pq_text_aligns[align_key] then
      table.insert(args, "text-align: " .. pq_text_aligns[align_key].css)
    end
    if boxalign_key and pq_box_aligns[boxalign_key] then
      table.insert(args, "box-align: " .. pq_box_aligns[boxalign_key].typst)
    end

    local arg_str = #args > 0 and (table.concat(args, ", ") .. ", ") or ""

    local typst_injections = ""
    ...
    return {
      pandoc.RawBlock('typst', '#pullquote(' .. arg_str .. ')[\n' .. typst_injections),
      el,
...
return {
  {
    Pandoc = function(doc)
      -- Read global document font configurations and map to typst_fonts
