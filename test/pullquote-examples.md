...
The extension provides a sizing scale to adjust the text relative to the document’s base font size. While the filter supports a full 9-step scale for API consistency, the larger sizes below are the most practical for pullquotes. If omitted, it defaults to the `pq-size-l` (large) scale.
...
The extension provides predefined typographic utility classes. These styles are mapped to equivalent rendering properties across all formats to ensure consistent output, regardless of the underlying engine.
...
Use `color` for the text, `barcolor` for the left border, and `barwidth` to adjust the border's thickness.
...
By adjusting `width` and `boxalign`, we can position the entire pullquote block to the left, center, or right of the page. This shifts the block within the standard document flow; the body text will sit above and below it rather than wrapping around it.
...
*(Absolute units such as `width="300pt"` or `width="10cm"` are also fully supported).*
