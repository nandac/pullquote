...
- Typst output defaulted its sans-serif font to `DejaVu Sans Mono` (a monospace font) in v1.0.0; it now uses a fallback chain (`Noto Sans`, `DejaVu Sans`, `Liberation Sans`, `Arial`, `Helvetica`) and warns to `stderr` when compiling to Typst without an explicit sans font configured (`sansfont` or `pq-family-sans`).
...
- Documented `pq-family-serif` and `pq-family-mono`, which have existed since v1.0.0 (mirroring the already-documented `pq-family-sans`) but were never mentioned in the README. Both are Typst-only, metadata-level overrides for the font a pullquote uses with `pq-family="serif"`/`"mono"`, falling back to `mainfont`/`monofont`/`codefont` if unset.
